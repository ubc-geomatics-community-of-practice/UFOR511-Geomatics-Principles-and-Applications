...
You will be submitting the following: - Answers to any questions posed
in the lab handout submitted through the quiz on Canvas. - A pdf map
complete with all necessary elements (scale, compass, legend, etc.)
...
**Step 2:** Right-click street-trees and select "Export Table." -
Input Rows: street-trees - Output Location: Where you want the table to
be saved - Output Name: AssignedTrees - Expression: + New expression

Where `Assigned` is equal to Y.

**Step 3:** Open the attribute table for **AssignedTrees** and add a new text field to the data. On the top of the attribute table, there is a button that says "Add". The table will switch to field view. Name the field name "Address", name the alias also "Address" and select "Text" as the data type. Click "Save" in the top bar. Close "Field View" and return to the attribute table. Your new field should appear as the very last column in the dataset. Right-click the field and select "Calculate Field."
...
**Step 1:** Create an address locator, which is located in Analysis \>
...
**Step 2:** Select Locate underneath the Map tab, and then click on
...
You can also use the Locate tool to Search an address using the ArcGIS
World Geocoding Service. Type in the address of Queen Elizabeth Park
(4600 Cambie St, Vancouver) and notice the map highlights near the park!
The ArcGIS World Geocoding Service will consume credits through your
ArcGIS account, so it is not the most cost-effective way to geocode many
addresses.
...
**Step 3:** Geocode a table of addresses using GVRDaddresses. The table
...
Below are the tool's parameters: - Input Table: AssignedTrees - Input
Address Locator: GVRDaddresses - Input Address Fields: Single Field -
Field Name: Full Address - Alias Name: Address

Save the file as **geocoded**.

**Step 4:** Look at the attribute table. The score column shows from
...
**Step 5:** Toggle the **AssignedTrees** and **geocoded** datasets on
...
right-hand side of Coordinate System and locating NAD 1983 UTM Zone 10N
...
Right-click **parks_polygon_representation**, mouse down to Joins and Relates \> Spatial
Join: - Target Features: parks_polygon_representation - Join Features:
parktrees - Output Feature Class: parks_numtrees -
Join Operation: Join one to one - Match Option: Intersect

If you cannot find this tool through the method above, then from the top ribbon, click Analysis, then click Tools and in the Geoprocessing Pane that opens, search for the tool named "Spatial Join".
...
Add a new field called "treedensity". Select "Float" from the drop-down menu for the data type--the data in this field will be continuous values and include
decimals. Now you can calculate the field, like you did in the previous steps. Tree density is the ratio of trees (JOIN_COUNT) to park area (area_ha). We will do
this calculation only for parks greater than or equal to one hectare in
size.
...
Requirements: - Parks symbolized by tree density - Coordinate-created
points and address-created points (symbolized differently from each
other!) - Roads symbolized - Necessary map components (title, legend,
scale bar, north arrow, student name and number, coordinate system)
